...
默认情况下，集群中的 pods 对外部网络是不可见的，但 kubectl 可以创建一个能够转发请求到集群端私有网络的代理：`kubectl proxy`。

```

```

注意：执行 `kubectl proxy` 后，当前终端会被阻塞，且不会有任何输出，执行 “ctrl + C” 可退出该命令。
...
<!--stackedit_data:
eyJoaXN0b3J5IjpbLTkzMzU1ODg2MV19
-->